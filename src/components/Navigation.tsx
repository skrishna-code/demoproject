          ...
          <div className="hidden md:flex space-x-1">
            {tabs.map((tab, index) => (
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 btn-hover animate-slideInRight stagger-${index + 1} ${
                  activeTab === tab.id
        ...
        <div className="md:hidden flex overflow-x-auto pb-2 space-x-1">
          {tabs.map((tab, index) => (
            <button
              key={tab.id}
              onClick={() => onTabChange(tab.id)}
              className={`flex flex-col items-center space-y-1 px-3 py-2 rounded-lg font-medium transition-all duration-200 whitespace-nowrap animate-fadeInUp stagger-${index + 1} ${
                activeTab === tab.id
                  ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg'
                  : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'
              }`}
